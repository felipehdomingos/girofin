...
function categoriaDe(desc: string): string | null {
  const g = guessCategory(desc);
  return g.categoryId ? (byId.get(g.categoryId) ?? null) : null;
}

check("ifood -> Restaurantes", categoriaDe("ifood almoco"), "Restaurantes");
check("uber -> Transporte", categoriaDe("uber para o trabalho"), "Transporte");
check("netflix -> Assinaturas", categoriaDe("netflix"), "Assinaturas");
check("carrefour -> Mercado", categoriaDe("carrefour compras"), "Mercado");
check("aluguel -> Moradia", categoriaDe("aluguel do mes"), "Moradia");
check("farmacia -> Saude", categoriaDe("farmacia remedio"), "Saúde");
check("desconhecido -> nenhum", categoriaDe("zxcvbnm qwerty"), null);
// "gas" não pode casar dentro de "gastos" — por isso o \b no regex.
check("gas nao casa dentro de gastos", categoriaDe("gastos diversos"), null);
...
const virgula = parseBulk("uber 28,50, ifood 45", categories);
...
const comNumero = parseBulk("99 pop 18,50", categories);
...
check(
  "compra no dia exato do fechamento ainda entra",
  firstInvoiceDueDate("2026-09-25", 25, 5),
  "2026-10-05",
);
// Cartão que fecha dia 1 e vence dia 10 do MESMO mês.
check(
  "vencimento depois do fechamento, mesmo mes",
